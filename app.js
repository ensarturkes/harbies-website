...
(function () {
  ...
  if (rot) {
    ...
    setInterval(function () {
      spans[ri].classList.remove('active');
      ri = (ri + 1) % spans.length;
      spans[ri].classList.add('active');
    }, 2800);
  }

  /* ---------- SES — Social Impact Counter ---------- */
  ...
  function setCount(n) { if (elCount) elCount.textContent = fmt(n); }

  paintMini();
  paintMeta();
  setCount(toys);

  // count-up animation when meter first enters view
  if (elMeter && elCount && 'IntersectionObserver' in window &&
  ...
  function toast(msg) {
    if (!elToast) return;
    elToast.innerHTML = '<svg viewBox="0 0 24 24" width="16" height="16" fill="var(--pudra)"><path d="M12 21s-7-4.35-9.5-8.5C.8 9 2.3 5.5 5.5 5.5c1.9 0 3.2 1 3.9 2.2C10.3 6.5 11.6 5.5 13.5 5.5c3.2 0 4.7 3.5 3 7C16 16.65 12 21 12 21z"/></svg>' + msg;
  ...
  function heartPop() {
    if (!elMeter || window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
